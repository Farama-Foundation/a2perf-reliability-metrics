...
def get_all_valid_eval_points(curves, window_size=1):
    ...
    logging.info(f'timepoints_all_curves: {timepoints_all_curves}')

    common_timepoints = functools.reduce(np.intersect1d, timepoints_all_curves)

    if len(common_timepoints) == 0:
        logging.info('common_timepoints was empty, so we\'ll choose all eval points from the first curve, '
                     'then find the closest timepoints on the other curves')
        common_timepoints = curves[0][0, :]
...
def divide_by_baseline(curves, baselines):
    ...
    if isinstance(baselines, list) or isinstance(baselines, tuple) or isinstance(
        ...
        for curve, baseline in zip(curves, baselines):
            curves_final.extend(
                apply_fn_to_curves([curve], lambda c: c / baseline))  # pylint: disable=cell-var-from-loop
    else:
...
def across_runs_preprocess(curves, eval_points, window_size, lowpass_thresh):
    ...
    logging.info(f'Curves supplied to across runs_preprocessing: {curves}')
    if eval_points is None or len(eval_points) == 0:
        logging.info('Eval points were not supplied for across runs metric')

        # Get the eval points by finding the timepoints common to all curves.
        eval_points_new = get_all_valid_eval_points(curves, window_size=1)
        logging.info(f'Length of eval_points_new: {len(eval_points_new)}')

        if len(eval_points_new) == 0:
            # no eval points in common, so we'll choose all eval points from the first curve,
            # then find the closest timepoints on the other curves
            logging.info('No eval points in common, so we\'ll choose all eval points from the first curve, '
                         'then find the closest timepoints on the other curves')
            eval_points_new = curves[0][0, :]
    else:
        logging.info('eval points was not None')
    ...
    logging.info('Shapes of filtered curves: %s', [curve.shape for curve in curves_filtered])
...
def get_nearest_within_window(curves, eval_points, window_size):
    ...
    for icurve, curve in enumerate(curves):
        ...
        logging.info(f'eval_points: {eval_points}')
        ...
        if np.any(distances >= half_window):
            ...
            raise ValueError(f'For curve {icurve}, there is no timepoint within window '
                             f'of size {window_size} around eval point {offending_eval_points}')
